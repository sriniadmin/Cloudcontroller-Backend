...
async function createDevice(req, res, next) {
    ...
    try {
        ...
        const check_number = await db_check_duplicate_device(condition, { transaction: t })
        ...
        if(params.patch_type === 'gateway'){
            if(params.sim){
                ...
                const check_sim = await db_check_duplicate_device(condition, { transaction: t })
                if(check_sim){
            ...
            if(params.phone){
                ...
                const check_phone = await db_check_duplicate_device(condition, { transaction: t })
                if(check_phone){
...
async function recall(length, number, req, res, next, transaction) {
    try {
        ...
        const check_tags = await db_check_duplicate_device(condition, { transaction: transaction })
        if (check_tags) {
            req.apiRes = {
        ...
        if (length === number) {
            ...
            const data = await db_create_device(req,  { transaction: transaction })
            req.apiRes = PATCH_CODE["3"]
            req.apiRes["response"] = { data: data }
            res.response(req.apiRes)
